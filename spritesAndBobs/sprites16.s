...
;Affichage et déplacement d'un sprite en 16 couleurs composé des sprites 0 et 1 sur un un bitplane (noter que BPL2MOD n'est pas utilisé car un unique bitplane, forcément impair car il s'agit donc du bitplane 1, est utilisé).
...
DISPLAY_Y=$2C
DISPLAY_DEPTH=1
COPPERLIST=9*4+DISPLAY_DEPTH*2*4+18*4+8*2*4+4
	;9*4					Configuration de l'affichage
	;DISPLAY_DEPTH*2*4		Adresses des bitplanes
	;18*4					Palette (couleurs 0-1 pour le bitplane, 16-31 pour le sprite)
	;8*2*4					Adresses des sprites
	;4						$FFFFFFFE
SPRITE_X=DISPLAY_X+14		;SPRITE_X-1 sera codé, car l'affichage des bitplanes est retardé d'un pixels sur celui des sprites par le hardware (non documenté)
...
_loop:
	...
	;Mettre à jour la position des sprites
...
sprite1:
					DC.W ((SPRITE_Y&$FF)<<8)!(((SPRITE_X-1)&$1FE)>>1)
					DC.W (((SPRITE_Y+SPRITE_DY)&$FF)<<8)!((SPRITE_Y&$100)>>6)!(((SPRITE_Y+SPRITE_DY)&$100)>>7)!((SPRITE_X-1)&$1)!$0080
					REPT 4
					DC.W $0000, $0000
					ENDR
					REPT 4
...
spriteVoid:
					DC.W 0, 0
spritePalette:
					DC.W $0000, $0F00, $0F50, $0FA0, $0FF0, $0080, $07C0, $00F0, $000F, $007F, $00FF, $080F, $0F0F, $0F8F, $0000, $0FFF
