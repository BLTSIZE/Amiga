...
;"Vector balls" avec de BOBs de 16 x 16 pixels en 4 couleurs. Version pour la capture d'écran uniquement (damier dans le fond, sphère supplénentaires, lignes entre certaines sphères), qui n'apporte rien de plus. La limitation de l'effacement à la zone utilisée et la parallélisation des transformations et de l'effacement ne permettent pas de tenir dans la trame sur A500 : il faudrait moins de sphères.
...
SIDE=100
DEPTH=140			;Distance entre l'observateur et l'écran
NBBOBS=43
ROTATEX=1
ROTATEY=2
ROTATEZ=1
TRANSLATEZ=225		;Distance entre l'observateur et l'origine du repère
...
;********** Programme principal **********
	...
	;Dessiner un damier dans le front buffer et le back buffer en couleur 1
...
_drawCheckerX:
	...
	lea (DISPLAY_DEPTH-1)*(DISPLAY_DX>>3)(a0),a0
	lea (DISPLAY_DEPTH-1)*(DISPLAY_DX>>3)(a1),a1
...
_swapBuffers:
	...
	;Effacer la partie du back buffer utilisée par les vector balls

	move.w #$0100,BLTCON0(a5)	;USEA=0, USEB=0, USEC=0, USED=1, D=0
	move.w #$0000,BLTCON1(a5)
	move.w #(6*16)>>3,BLTDMOD(a5)
	movea.l backBuffer,a0
	lea 16*DISPLAY_DEPTH*(DISPLAY_DX>>3)+((3*16)>>3)(a0),a0
	move.l a0,BLTDPTH(a5)
	move.w #((DISPLAY_DEPTH*(DISPLAY_DY-16*2))<<6)!((DISPLAY_DX-6*16)>>4),BLTSIZE(a5)
...
_angleZNoUnderflow:
	...
	;Tracer des lignes entre les BOBs formant un cube (à faire avant le tri !)

	WAIT_BLITTER
	movea.l backBuffer,a2
	lea DISPLAY_DX>>3(a2),a2	;Tracer en COLOR01
...
bobs3D:
					...
					DC.W -SIDE/2, -SIDE/2, SIDE/2
					DC.W -SIDE/2, SIDE/2, SIDE/2
					DC.W 3*SIDE/4, 3*SIDE/4, -3*SIDE/4
					DC.W 3*SIDE/4, -3*SIDE/4, -3*SIDE/4
					DC.W -3*SIDE/4, -3*SIDE/4, -3*SIDE/4
					DC.W -3*SIDE/4, 3*SIDE/4, -3*SIDE/4
					DC.W 3*SIDE/4, 3*SIDE/4, 3*SIDE/4
