...
;Lecture d'une touche pressée au clavier (mode interruption)
...
;Initialisation de la mécanique d'interruption par le CIA A sur réception des bits transmis par le clavier. On choisit de ménager le système d'exploitation en n'inhibant pas la possibilité pour le CIA A de présenter d'autres requêtes d'interruption que celle qui nous intéresse (autrement dit, pas de move.b #$1F,$BFED01 pour commencer)
	...
	move.w #$0008,INTREQ(a5)	;Acquitter l'éventuelle requête d'interruption de niveau 2 (bit PORTS)
	movea.l VBRPointer,a0
	move.l #_keyboard,$68(a0)	;Installer le gestionnaire d'interruption à l'adresse du vecteur de l'interuption niveau 2
	move.w #$C008,INTENA(a5)	;Activer la possibilité pour une requête d'interruption d'entraîner effectivement une interruption du CPU (bits INTEN et PORTS)
...
_loop:
	...
	move.w #$4008,INTENA(a5)	;Inhiber la possibilité pour une requête d'interruption d'entraîner effectivmeent une interruption du CPU
...
;Au minimum, le gestionnaire d'interruption doit acquitter la requête d'interruption de niveau 2 (effacer le bit PORTS dans INTREQ) et l'interruption sous-jacente du CIA A qui en est à l'origine (accès en lecture sur CIA A ICR qui acquitte tous les bits dans ICR). Bien évidemment, ce minimum s'entend que sous la condition que la réception des 8 bits transmis par le clavier constitue l'événement générateur de la requête d'interruption de niveau 2 :
;	tst.b $BFED01
;	move.w #$0008,INTREQ(a5)
;	rte

_keyboard:
	movem.l d0-d2,-(sp)		;Indispensable, mais à limiter aux registres modifiés
...
_keyboardNotKeyboard:
	...
	;Acquitter la requête d'interruption

	move.w #$0008,INTREQ(a5)
	movem.l (sp)+,d0-d2		;Indispensable, mais à limiter aux registres modifiés
	rte
